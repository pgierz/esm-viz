...
class slurm:
    ...
    def update_queue_info(self):
        ...
        squeue_output = subprocess.run(
            ["squeue", "-u", username, "-o", "%all"], stdout=subprocess.PIPE
        )
        squeue_output_formatted = [
            i.split("|") for i in squeue_output.stdout.decode("utf-8").split("\n")
        ]
    ...
    def __repr__(self):
        return self.queue_status[
            ["JOBID", "PARTITION", "USER", "NAME", "TIME"]
        ].to_string()

    def _repr_html_(self):
        return self.queue_status[
            ["JOBID", "PARTITION", "USER", "NAME", "STATE", "TIME", "START_TIME"]
        ].to_html()
